...
const MonthlyExport: React.FC<{ transactions: Transaction[] }> = ({ transactions }) => {
    const handleExport = () => {
        ...
        // Use UTC date parts for ISO 8601 format to avoid timezone issues
        const toISODateString = (dateString: string) => {
            return new Date(dateString).toISOString().split('T')[0];
        };

        const headers = ["id", "tanggal", "tipe", "kategori", "deskripsi", "jumlah", "sumber_dana", "ref_payment", "nota_url", "created_by"];
        const csvContent = [
            headers.join(','),
            ...monthlyTransactions.map(t => [
                t.id,
                toISODateString(t.tanggal),
                t.tipe,
                `"${t.kategori.replace(/"/g, '""')}"`,
                `"${t.deskripsi.replace(/"/g, '""')}"`,
                t.jumlah, // Pure number
                t.sumber_dana,
                t.ref_payment || '',
                t.nota_url || '',
                t.created_by || ''
        ...
        if (link.download !== undefined) {
            ...
            link.setAttribute('download', `laporan_kas_${currentYear}-${String(currentMonth + 1).padStart(2, '0')}.csv`);
    ...
    return (
         <div>
            ...
            <p className="text-sm text-gray-600 mb-4">Ekspor semua transaksi bulan ini ke dalam format CSV (tanggal dalam format YYYY-MM-DD, jumlah sebagai angka).</p>
            <button onClick={handleExport} className="w-full py-2 px-4 bg-gray-700 text-white font-semibold rounded-md hover:bg-gray-800">Ekspor CSV Bulan Ini</button>
        </div>
    );
};
...
const AdminPanel: React.FC<{
  ...
  onBulkAdd: (students: Omit<Student, 'id' | 'created_at'>[]) => Promise<void>;
    ...
    const TABS = ['Tambah Pembayaran', 'Tambah Pemasukan', 'Tambah Pengeluaran', 'Manajemen Mahasiswa', 'Tambah Massal', 'Manajemen Transaksi', 'Ekspor Bulanan'];
